...
class TyprU {
    ...
    static _drawGlyf(gid, font, path) {
        ...
        if (gl == null)
            gl = font.glyf[gid] = Parsers.Glyf._parseGlyf(font, gid);
        if (gl != null) {
            if (gl.noc > -1)
                TyprU._simpleGlyph(gl, path);
            else
                TyprU._compoGlyph(gl, font, path);
        }
    }
                ...
                if (i == i0) {
                    if (onCurve) {
                        ...
                        if (prOnCurve)
                            ...
                            P.moveTo(p, (gl.xs[pr] + x) / 2, (gl.ys[pr] + y) / 2);
                    }
                }
                if (onCurve) {
                    if (prOnCurve)
                    ...
                    if (nxOnCurve)
                        P.qcurveTo(p, x, y, gl.xs[nx], gl.ys[nx]);
                    else
                        P.qcurveTo(p, x, y, (x + gl.xs[nx]) / 2, (y + gl.ys[nx]) / 2);
                }
            }
            P.closePath(p);
        }
    }
    static _compGlyph(gl, font, p) {
        for (let j = 0; j < gl.parts.length; j++) {
            let path = { cmds: [], crds: [] };
            let prt = gl.parts[j];
            TyprU._drawGlyf(prt.glyphIndex, font, path);
    ...
    static getPairAdjustment(font, g1, g2) {
        if (font.GPOS) {
            let ltab = null;
    ...
    static glyphsToPath(font, gls, clr) {
        ...
        for (let i = 0; i < gls.length; i++) {
            ...
            if (i < gls.length - 1)
                x += TyprU.getPairAdjustment(font, gid, gid2);
        }
        return tpath;
    }
    ...
    static _drawCFF(cmds, state, font, p) {
...
exports.TyprU = TyprU;
//# sourceMappingURL=Typr.U.js.map