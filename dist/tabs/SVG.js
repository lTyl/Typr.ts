...
class SVG {
    static parse(data, offset, length) {
        let obj = { entries: [] };
        let offset0 = offset;
        let tableVersion = core_1.Bin.readUshort(data, offset);
        offset += 2;
        let svgDocIndexOffset = core_1.Bin.readUint(data, offset);
        offset += 4;
        let reserved = core_1.Bin.readUint(data, offset);
        offset += 4;
        offset = svgDocIndexOffset + offset0;
        let numEntries = core_1.Bin.readUshort(data, offset);
        offset += 2;
        for (let i = 0; i < numEntries; i++) {
            let startGlyphID = core_1.Bin.readUshort(data, offset);
            offset += 2;
            let endGlyphID = core_1.Bin.readUshort(data, offset);
            offset += 2;
            let svgDocOffset = core_1.Bin.readUint(data, offset);
            offset += 4;
            let svgDocLength = core_1.Bin.readUint(data, offset);
            offset += 4;
            let sbuf = new Uint8Array(data.buffer, offset0 + svgDocOffset + svgDocIndexOffset, svgDocLength);
            let svg = core_1.Bin.readUTF8(sbuf, 0, sbuf.length);
    ...
    static _toPath(nds, pth, fill) {
        for (let ni = 0; ni < nds.length; ni++) {
            ...
            if (cfl == null)
                cfl = fill;
            if (tn == "g")
                SVG._toPath(nd.children, pth, cfl);
            else if (tn == "path") {
